        ...
        <main className="flex-1 p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {defaultRecommendations.map((rec) => (
              <div
                key={rec.id}
                className="glass-panel rounded-xl p-6 border-primary/30 shadow-glow flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-mono font-bold text-primary bg-primary-container/25 border border-primary/30 px-2 py-0.5 rounded uppercase">
                      {rec.priority}
                    </span>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center text-xs font-mono mb-4">
                    <div className="p-2.5 rounded bg-surface-dim border border-outline-variant">
                      <div className="text-[10px] text-on-surface-variant/70 mb-0.5">Projected Recovery</div>
                      <div className="font-extrabold text-primary">{formatCurrencyThousands(rec.recoveryUSD)}</div>
                    </div>
                    <div className="p-2.5 rounded bg-surface-dim border border-outline-variant">
                      <div className="text-[10px] text-on-surface-variant/70 mb-0.5">Confidence</div>
                      <div className="font-extrabold text-on-surface">{rec.confidence}% HIGH</div>
                    </div>
                    <div className="p-2.5 rounded bg-surface-dim border border-outline-variant">
                      <div className="text-[10px] text-on-surface-variant/70 mb-0.5">Timeframe</div>
                      <div className="font-extrabold text-secondary">{rec.timeframe}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    {rec.levers.map((lever, idx) => (
                      <div
                        key={idx}
                        className="p-2 rounded bg-surface-dim/80 border border-outline-variant/60 flex items-start gap-2 text-xs"
                      >
          ...
          <div className="glass-panel rounded-xl p-6 md:p-8 border-primary/40 bg-gradient-to-r from-primary-container/15 via-surface-container to-surface shadow-glow">
            <div className="flex flex-col md:flex-row md:items-center justify-between pb-5 border-b border-outline-variant gap-4 mb-6">
              ...
              <span className="text-xs font-mono text-primary bg-primary-container/20 border border-primary/30 px-3 py-1 rounded-full font-bold self-start md:self-auto">
                Elasticity Ratio: 0.73 • Deterministic Truth
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              ...
              <div className="lg:col-span-6 space-y-4">
                <div className="flex justify-between items-center text-xs font-mono">
                  ...
                  <span className="font-display font-extrabold text-2xl text-primary">{sliderVal.toFixed(1)}%</span>
                </div>
                ...
                <div className="flex justify-between text-[11px] font-mono text-on-surface-variant/80">
                  ...
                  <span className="text-primary font-bold">Target: 90.0%</span>
                ...
                <div className="pt-3 border-t border-outline-variant/40 space-y-1 text-[11px] font-mono text-on-surface-variant/70">
                  <div className="font-bold text-on-surface-variant mb-1">Model Assumptions:</div>
